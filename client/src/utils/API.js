...
export const getShow = function (id, token) {
  return axios.get(`/api/users/shows/${id}`, { headers: { authorization: `Bearer ${token}` } });
}

export const deleteShow = function (id, token) {
  return axios.delete(`/api/users/shows/${id}`, { headers: { authorization: `Bearer ${token}` } });
}

export const updateShow = function (id, token, showData){
  return axios.put(`/api/users/shows/${id}`, showData, { headers: { authorization: `Bearer ${token}` } });
}
